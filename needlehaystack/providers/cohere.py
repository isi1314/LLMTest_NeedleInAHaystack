...
class Cohere(ModelProvider):
    DEFAULT_MODEL_KWARGS: dict = dict(max_tokens=50, temperature=0.3)

    def __init__(
        self, model_name: str = "command-r", model_kwargs: dict = DEFAULT_MODEL_KWARGS
    ):
        """
    ...
    def decode_tokens(
        ...
        return (
            Client()
            .detokenize(tokens=tokens[:context_length], model=self.model_name)
            .text
        )
    ...
    def get_langchain_runnable(self, context: str):
        ...
        api_key = os.getenv("NIAH_MODEL_API_KEY")
        model = ChatCohere(
            cohere_api_key=api_key, temperature=0.3, model=self.model_name
        )
